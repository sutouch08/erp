...
if( isset( $_GET['confirmOrder'] ) && isset( $_POST['id_order'] ) )
{ 
	...
	if( $state != 10)
	{
		$sc = 'สถานะของออเดอร์ถูกเปลี่ยนไปแล้ว';
	}
	else
	{
		...
		if( $order->role == 5 )
		{
			...
			if( $id_zone !== FALSE )
			{
				$qr = "SELECT tbl_qc.id_product_attribute, SUM(tbl_qc.qty) AS qty, id_zone, id_warehouse FROM tbl_qc ";
				$qr .= "JOIN tbl_temp ON tbl_qc.id_temp = tbl_temp.id_temp ";
				...
				while( $rs = dbFetchObject($qs) )
				{
					...
					if( $ra !== TRUE || $rb !== TRUE || $rc !== TRUE || $re !== TRUE )
					{ 
						$success = FALSE; 
					}
				}
		...
		{		
			...
			while( $rs = dbFetchObject($qs) )
			{
				...
				$product 	= new product();
				$id_product = $product->getProductId($id_pa);
				$ra = dbQuery("UPDATE tbl_temp SET status = 4  WHERE id_temp IN( SELECT id_temp FROM (SELECT id_temp FROM tbl_temp WHERE id_order = ".$id_order." AND id_product_attribute = ".$id_pa." LIMIT ".$qty.") tmp)"); 
				$rb = update_buffer_zone($new_qty, $id_product, $id_pa, $id_order, $from_zone, $id_warehouse, $id_employee);
				$rc = stock_movement("out", 3, $id_pa, $id_warehouse, $qty, $order->reference, $date_upd, $from_zone);
...
{
	$cookie  = array('sReference', 'sCustomer', 'fromDate', 'toDate');
