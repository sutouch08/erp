...
echo date('H:i:s') . '=> Start query'.EOL;
$role = $_GET['role'];
$from = fromDate($_GET['fromDate']);
$to   = toDate($_GET['toDate']);

$qr  = "SELECT ods.*, w.name AS wh_name FROM tbl_order_sold AS ods ";
$qr .= "LEFT JOIN tbl_warehouse AS w ON ods.id_warehouse = w.id ";
$qr .= "WHERE ods.date_add >= '".$from."' AND ods.date_add <= '".$to."' ";
if( $role == 0 )
{
  $qr .= "AND ods.id_role IN(1,8) ";
}
else
{
  $qr .= "AND ods.id_role = ".$role." ";
}

$qr .= "ORDER BY ods.date_add ASC, ods.product_code ASC";


//echo $qr;
$qs = dbQuery($qr);
echo date('H:i:s') . ' = > Finished Query'.EOL;
echo date('H:i:s'). ' => Start Writh Data to Memory'.EOL;
...
if(dbNumRows($qs) > 0)
{
  ...
  while($rs = dbFetchObject($qs))
  {
    ...
    $excel->getActiveSheet()->setCellValue('AQ'.$row, $rs->wh_name);
...
echo date('H:i:s') . ' => Finished Write Data To RAM'.EOL;
echo 'Write Data TO RAM : ' . sprintf('%.4f',$callTime) . " seconds" . EOL;
