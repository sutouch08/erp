	...
	function exportBI($id_receive_product)
	{
		...
		if( dbNumRows($qs) > 0 )
		{
			while( $rs = dbFetchObject($qs) )
			{
				...
				$excel->getActiveSheet()->setCellValue('AK'.$row, $po->getPrice($cs->po, $rs->id_product));
				$excel->getActiveSheet()->setCellValue('AL'.$row, $po->getDiscount($cs->po, $rs->id_product));
		...
		$writer = PHPExcel_IOFactory::createWriter($excel, 'Excel5');//new PHPExcel_IOFactory::createWriter($excel, 'Excel5');
		try
		{
