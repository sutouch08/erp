...
if( isset( $_GET['getTopTable'] ) )
{
	...
	if( dbNumRows($qs) > 0 )
	{
		...
		while( $rs = dbFetchArray($qs)	)
		{
			...
			$inZone		= $product->stockInZone($id_pa);
			$arr = array(
						"id_pa"		=> $id_pa,
...
if( isset( $_GET['perparedItem'] ) )
{
	...
	$auz				= isAllowUnderZero($id_wh);
	$canPrepare	= isAllowPrepare($id_wh);
	$whActive		= isWarehouseActive($id_wh);
	if( $valid === TRUE )
	{
		...
		//------------- ถ้าคลังนี้ไม่สามารถจัดสินค้าได้ -----///
		if( $canPrepare === FALSE OR $whActive === FALSE )
		{
			$sc = 'ไม่อนุญาติให้จัดสินค้าในคลังนี้';
			$valid = FALSE;
		}
...
if( isset( $_GET['getZone'] ) )
{
	$sc = 'fail';
	$barcode 	= $_POST['barcode'];
	$qr = "SELECT id_zone FROM tbl_zone JOIN tbl_warehouse ON tbl_zone.id_warehouse = tbl_warehouse.id_warehouse ";
	$qr .= "WHERE tbl_warehouse.active = 1 AND tbl_warehouse.prepare = 1 AND barcode_zone = '".$barcode."'";
	$qs = dbQuery($qr);
	if( dbNumRows($qs) == 1 )
	{
