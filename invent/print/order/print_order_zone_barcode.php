...
// }
$qr  = "SELECT so.*, zone.barcode_zone FROM tbl_order_sold AS so ";
$qr .= "LEFT JOIN tbl_zone AS zone ON so.id_zone = zone.id_zone ";
$qr .= "WHERE so.id_order = {$order->id}";

$detail = dbQuery($qr); //$order->getSoldDetails($order->id);
...
$thead	= array(
          ...
          array("มูลค่า", "width:10%; text-align:center; border-left: solid 1px #ccc; border-top:0px;"),
...
while($total_page > 0 )
{
  ...
  while($i<$row)
  {
    ...
    if( ! empty($rs) )
    {
      ...
      $discount = $rs->discount_label; //isset( $rs->discount_label) ? $rs->discount_label : $rs->discount;
