...
?>	
	  <div class='row'>
		...
		<dl style='float:left; margin-left:10px;'>
            ...
            <dd style='float:left; margin:0px; padding-right:10px'><?php echo number_format($order->qc_qty()); ?></dd>
              |
        </dl>
...
<?php
	...
	$sCustomer		= isset( $_POST['sCustomer'] ) ? $_POST['sCustomer'] : ( getCookie('sCustomer') ? getCookie('sCustomer') : '' );
	$from				= isset( $_POST['fromDate'] ) ? $_POST['fromDate'] : ( getCookie('fromDate') ? getCookie('fromDate') : '' );
	$to				= isset( $_POST['toDate'] ) ? $_POST['toDate'] : ( getCookie('toDate') ? getCookie('toDate') : '' );
...
<div class="row">
	<div class="col-sm-3">
    	<label>เอกสาร</label>
        <input type="text" class="form-control input-sm" name="sReference" id="sReference" value="<?php echo $sReference; ?>" />
    </div>
    
    <div class="col-sm-3">
    	<label>ลูกค้า</label>
        <input type="text" class="form-control input-sm" name="sCustomer" id="sCustomer" value="<?php echo $sCustomer; ?>" />
    </div>
    
    <div class="col-sm-3">
    	<label class="display-block">วันที่</label>
        <input type="text" class="form-control input-sm input-discount text-center" name="fromDate" id="fromDate" placeholder="เริ่มต้น" value="<?php echo $from; ?>" />
        <input type="text" class="form-control input-sm input-unit text-center" name="toDate" id="toDate" placeholder="สิ้นสุด" value="<?php echo $to; ?>" />
    </div>
    ...
    <div class="col-sm-1 padding-5">
    	<label class="display-block not-show">submit</label>
        <button type="button" class="btn btn-sm btn-warning btn-block" onclick="clearFilter()"><i class="fa fa-retweet"></i> รีเซ็ต</button>
    </div>
</div>
</form>
<hr class="margin-top-15 margin-bottom-15"/>
<?php
	$where = "WHERE current_state = 10 ";
	if( $sReference != '' )
	{
	...
	if( $sCustomer != '' )
	{
		...
		$cus	= customer_in($sCustomer);
		if( $cus !== FALSE )
		{
			$where .= "AND id_customer IN(".$cus.") ";	
		}
		else
		{
	...
	if( $from != '' && $to != '' )
	{
		createCookie('fromDate', $from);
		createCookie('toDate', $to);
    	...
    	<table class="table table-striped" style="border:solid 1px #CCC;">
        	<thead>
            	<tr>
                    ...
                    <th class="width-10 text-center">เงื่อนไข</th>
                    <th class="width-10">พนักงาน</th>
...
<?php		while( $rs = dbFetchObject($qs) ) : ?>
				<tr style="font-size:12px;">
                	<td align="center" class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo $no; ?></td>
                    <td class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo $rs->reference; ?></td>
                    <td class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo customer_name($rs->id_customer); ?></td>
                    <td align="center" class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo number_format(getBillAmount($rs->id_order), 2); ?></td>
                    <td align="center" class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo $rs->payment; ?></td>
                    <td class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo employee_name($rs->id_employee); ?></td>
                    <td align="center" class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo thaiDate($rs->date_add, '/'); ?></td>
                    <td align="center" class="pointer" onclick="viewDetail(<?php echo $rs->id_order; ?>)"><?php echo thaiDate($rs->date_upd, '/'); ?></td>
                </tr>
<?php		$no++; ?>                
<?php 		endwhile; ?>
<?php	else : ?>
			<tr><td colspan="8" align="center"><h4>ไม่มีรายการตามเงื่อนไขที่กำหนด</h4></td></tr>
<?php  	endif; ?>       
			</tbody>           
        </table>
    </div>
...
function getSearch()
{
	var sRef = $("#sReference").val();	
	var sCus	= $("#sCustomer").val();
	var from	= $("#fromDate").val();
	var to		= $("#toDate").val();
	if( sRef != "" || sCus != "" || ( from != "" && to != "") )
	{
		$("#searchForm").submit();
	}
}
...
{
	$.ajax({
		url:"controller/billController.php?clearFilter",
...
{
	window.location.href = 'index.php?content=bill&view_detail=y&id_order='+id_order;	
}
...
function save_iv(){
	...
	$.ajax({
		...
		type:"POST", cache:"false", data:{ "id_order" : id_order, "id_employee" : id_employee }, 
		success: function(rs){
			load_out();
			var rs = $.trim(rs);
			if( rs == 'success' ){
				swal({title: 'success', text: 'เปิดบิลและตัดสต็อกเรียบร้อยแล้ว', type: 'success', timer: 1000});
