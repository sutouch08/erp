...
<div class="row">
	<div class="col-sm-12">
        ...
        <?php		$inZone		= $product->stockInZone($id_pa); // $product->stock_in_zone($id_pa);	?>
        <tr style="font-size:12px;">
            ...
            <td align="center">
            <button type="button" class="btn btn-sm btn-default btn-pop" data-container="body" data-toggle="popover" data-placement="right" data-html="true" data-content="<?php echo $inZone; ?>" <?php echo $notShow; ?>>
            ที่เก็บ
            </button>
...
<?php 	while($rs = dbFetchArray($qs)) : ?>
	<tr>
        ...
        <td align="right">
            <a href="controller/prepareController.php?bring_it_back&id_prepare=<?php echo $rs['id_prepare']; ?>">
            	<button type="button" class="btn btn-warning"><i class="fa fa-refresh"></i>&nbsp; ดึงกลับ</button>
            </a>
        </td>
    </tr>
<?php $n++; ?>
	...
	$.ajax({
		...
		success: function(rs){
			...
			if( rs == 'fail' || isNaN(parseInt(rs)) ){
				swal({ 
						title: 'ข้อผิดพลาด',
						text: 'บาร์โค้ดโซนไม่ถูกต้อง หรือ ไม่อนุญาติให้จัดสินค้าในคลังนี้',
						type: 'error'
				});
				$("#barcode_zone").focus();
			}else{
