...
public function available_order_qty($id_product_attribute)
{
	...
	$qr = "SELECT SUM( qty ) AS qty FROM tbl_stock JOIN tbl_zone ON tbl_stock.id_zone = tbl_zone.id_zone ";
	$qr .= "JOIN tbl_warehouse ON tbl_zone.id_warehouse = tbl_warehouse.id_warehouse ";
	$qr .= "WHERE id_product_attribute = ".$id_product_attribute." AND tbl_warehouse.sell = 1 AND tbl_warehouse.active = 1";
	$sql = dbQuery($qr);
	$row = dbNumRows($sql);
	if($row > 0){
		$rs = dbFetchArray($sql);
...
public function available_product_qty($id_pd)
{
	...
	{
		$qs = "SELECT SUM(qty) FROM tbl_stock ";
		...
		$qs .= "JOIN tbl_warehouse ON tbl_zone.id_warehouse = tbl_warehouse.id_warehouse ";
		$qs .= "WHERE id_product = ".$id_pd." AND tbl_warehouse.sell = 1 AND tbl_warehouse.active = 1 AND qty > 0";
		$qs = dbQuery($qs);
		if(dbNumRows($qs) == 1 )
		{
...
private function orderAttributeGridOneAttribute($id_pd, $attr, $isVisual)
{
	...
	while($rs = dbFetchArray($data))
	{
		...
		$qty        	= $auz == 1 ? FALSE : ( $isVisual === FALSE ? ( $active == 1 ? $this->available_order_qty($id_pa) : FALSE ) : FALSE ); //---- ถ้าเป็นสินค้าเสมือนไม่ต้องมีสต็อก
		

		$disabled 	= $auz == 1 ? '' : ( $isVisual === TRUE  && $active == 1 ? '' : ($qty < 1 ? 'disabled' : '') ); 
		...
		while($rs = dbFetchArray($qs))
		{
			...
			while($rd = dbFetchArray($qr))
			{
				...
				if( $id_pa !== FALSE)
				{
					...
					$qty        	= $auz == 1 ? FALSE : ( $isVisual === FALSE && $active == 1 ? $this->available_order_qty($id_pa) : FALSE ) ; //---- ถ้าเป็นสินค้าเสมือนไม่ต้องมีสต็อก
					$disabled 	= $auz == 1 ? '' : ( $isVisual === TRUE && $active == 1 ? '' : ($qty < 1 ? 'disabled' : ( $active == 1 ? '' : 'disabled' ) ) ); 
...
public function no_field()
{
	...
	{
		$sc = "";
		$qr = "SELECT zone_name, SUM(qty) AS qty FROM tbl_stock ";
		$qr .= "JOIN tbl_zone ON tbl_stock.id_zone = tbl_zone.id_zone ";
		$qr .= "JOIN tbl_warehouse ON tbl_zone.id_warehouse = tbl_warehouse.id_warehouse ";
		$qr .= "WHERE tbl_stock.id_product_attribute = ".$id_pa." AND tbl_warehouse.prepare = 1 AND tbl_warehouse.active = 1 ";
		$qr .= "GROUP BY tbl_stock.id_zone ORDER BY tbl_zone.zone_name ASC";
